...
void mark_exams(struct semaphore *semaphores, int id, int *student_numbers, int *shared_index)
{
    ...
    while (rounds_completed < MARKING_ROUNDS)
    {
        ...
        // Attempt to acquire the first semaphore
        if (P(&semaphores[current]) != 0)
        {
            // Failed to acquire; retry later
            continue;
        }

        // Attempt to acquire the second semaphore
        if (P(&semaphores[next]) != 0)
        {
            // Failed to acquire second semaphore; release the first and retry later
        ...
        V(&semaphores[next]);
        V(&semaphores[current]);
        printf("TA %d released semaphores %d and %d.\n", id + 1, current, next);
        
        // Critical section: Mark student
